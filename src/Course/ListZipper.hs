...
instance Functor ListZipper where
  (<$>) f (ListZipper ls c rs) = ListZipper (f <$> ls) (f c) (f <$> rs)
...
findLeft p (ListZipper ls c rs) = case break p ls of
                                    (rs', c' :. ls') -> IsZ $ ListZipper ls' c' (reverse rs' ++ c :. rs)
                                    _ -> IsNotZ
...
findRight p (ListZipper ls c rs) = case break p rs of
                                     (ls', c' :. rs') -> IsZ $ ListZipper (reverse ls' ++ c :. ls) c' rs'
                                     _ -> IsNotZ
...
moveLeftLoop (ListZipper Nil c rs) = let (c' :. ls') = reverse (c :. rs)
                                     in ListZipper ls' c' Nil
moveLeftLoop (ListZipper (c' :. ls') c rs) = ListZipper ls' c' (c :. rs)
...
moveRightLoop (ListZipper ls c Nil) = let (c' :. rs') = reverse (c :. ls)
                                      in ListZipper Nil c' rs'
moveRightLoop (ListZipper ls c (c' :. rs')) = ListZipper (c :. ls) c' rs'
...
moveLeftN n | n == 0 = IsZ
            | n < 0 = moveRightN (- n)
            | otherwise = asMaybeZipper (moveLeftN (n - 1)) . moveLeft
...
moveRightN n | n == 0 = IsZ
             | n < 0 = moveLeftN (- n)
             | otherwise = asMaybeZipper (moveRightN (n - 1)) . moveRight
...
moveLeftN' n l@(ListZipper ls _ _) | n < 0 = moveRightN' (- n) l
                                   | otherwise = f $ moveLeftN n l
                                      where f IsNotZ   = Left $ length ls
                                            f (IsZ l') = Right l'
...
moveRightN' n l@(ListZipper _ _ rs) | n < 0     = moveLeftN' (- n) l
                                    | otherwise = f $ moveRightN n l
                                        where f IsNotZ   = Left $ length rs
                                              f (IsZ l') = Right l'
...
nth n l@(ListZipper ls _ _) = moveLeftN (length ls - n) l
...
end l@(ListZipper ls c rs) = case reverse rs of
                              Nil         -> l
                              (c' :. rs') -> ListZipper (rs' ++ c :. ls) c' Nil
...
start l@(ListZipper ls c rs) = case reverse ls of
                                Nil         -> l
                                (c' :. ls') -> ListZipper Nil c' (ls' ++ c :. rs)
...
deletePullLeft (ListZipper ls _ rs) = case ls of
                                       (c' :. ls') -> IsZ $ ListZipper ls' c' rs
                                       _           -> IsNotZ
...
deletePullRight (ListZipper ls _ rs) = case rs of
                                       (c' :. rs') -> IsZ $ ListZipper ls c' rs'
                                       _           -> IsNotZ
...
insertPushLeft a (ListZipper ls c rs) = ListZipper (c :. ls) a rs
...
insertPushRight a (ListZipper ls c rs) = ListZipper ls a (c :. rs)
...
-- /Tip:/ Use `zipWith`
  (<*>) (ListZipper ls c rs) (ListZipper ls' c' rs')
                                = ListZipper (zipWith ($) ls ls')
                                             (c c')
                                             (zipWith ($) rs rs')
...
instance Extend ListZipper where
  (<<=) f l = ListZipper ls (f l) rs
                where ls = unfoldr (f' moveLeft) l
                      rs = unfoldr (f' moveRight) l
                      f' g i = (\k -> (f k, k)) <$> toOptional (g i)
...
instance Traversable ListZipper where
  traverse f (ListZipper ls c rs) = ListZipper <$> traverse f ls <*> f c <*> traverse f rs
