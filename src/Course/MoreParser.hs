...
hex = (\s -> case readHex s of
              Full n -> pure (chr n)
              Empty  -> constantParser $ UnexpectedString s) =<< thisMany 4 (satisfy isHexDigit)
...
sepby1 p s = lift2 (:.) p (s *> sepby1 p s ||| pure Nil)
...
betweenSepbyComma d1 d2 p = betweenCharTok d1 d2 (sepby p (charTok ','))
