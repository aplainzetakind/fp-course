...
findM p = foldRight (\a foa -> (\b -> if b then pure (Full a) else foa) =<< p a) (pure Empty)
...
firstRepeat = flip eval S.empty . findM p
                      where p a = (\s -> const (pure (a `S.member` s)) =<< put (a `S.insert` s)) =<< get
...
distinct = flip eval S.empty . filtering f
             where f x = get >>= \s -> put (x `S.insert` s) >>= const (pure . not $ x `S.member` s)
...
isHappy = contains 1 . firstRepeat . produce next
            where next = toInteger . sum . ((join (*) . digitToInt) <$>) . show'
